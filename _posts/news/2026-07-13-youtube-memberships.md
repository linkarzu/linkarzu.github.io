---
title: Launch Your Own Podcast on My YouTube Channel
description: >-
  Become a Director, host your own monthly podcast, bring your own guests, reach
  my audience, and get a fully edited episode, five Shorts, a 10-minute clip,
  and distribution to Spotify and other podcast platforms.
image:
  path: ./../../assets/img/imgs/260714-thux-your-podcast.avif
date: '2026-07-14 06:10:00 +0000'
categories:
  - youtube
tags:
  - youtube
  - memberships
  - podcast
  - livestream
  - community
  - director
---
## Contents

### Table of contents

<!-- toc -->

- [Membership Levels](#membership-levels)
  * [Member](#member)
  * [Executive Producer](#executive-producer)
  * [CEO](#ceo)
  * [Board Member](#board-member)
  * [Podcast Director](#podcast-director)
- [Podcast Director information](#podcast-director-information)
  * [Important information](#important-information)
    + [What is included in the Podcast Director membership?](#what-is-included-in-the-podcast-director-membership)
    + [Why does the Podcast Director membership cost that?](#why-does-the-podcast-director-membership-cost-that)
    + [What are the benefits of becoming a Podcast Director?](#what-are-the-benefits-of-becoming-a-podcast-director)
  * [Running your podcast](#running-your-podcast)
    + [Who runs the podcast episode?](#who-runs-the-podcast-episode)
    + [How many guests can I invite?](#how-many-guests-can-i-invite)
    + [Can the podcast be about anything?](#can-the-podcast-be-about-anything)
    + [Can we record with or without a live audience?](#can-we-record-with-or-without-a-live-audience)
    + [What equipment do I and my guests need?](#what-equipment-do-i-and-my-guests-need)
  * [Publishing and editing](#publishing-and-editing)
    + [Where will my podcast be published?](#where-will-my-podcast-be-published)
    + [What editing is included?](#what-editing-is-included)
    + [What additional videos do I receive?](#what-additional-videos-do-i-receive)
    + [How long does publication take?](#how-long-does-publication-take)
    + [Can I publish the full podcast episode on my own YouTube channel?](#can-i-publish-the-full-podcast-episode-on-my-own-youtube-channel)
  * [Scheduling](#scheduling)
    + [How far in advance should I schedule the episode?](#how-far-in-advance-should-i-schedule-the-episode)
    + [What happens if I do not use my episode that month?](#what-happens-if-i-do-not-use-my-episode-that-month)
    + [What happens if I cancel the membership?](#what-happens-if-i-cancel-the-membership)
    + [Can I purchase an additional recording hour?](#can-i-purchase-an-additional-recording-hour)
  * [Final notes](#final-notes)
    + [Are views, subscribers, clients, or sales guaranteed?](#are-views-subscribers-clients-or-sales-guaranteed)
    + [Are refunds available?](#are-refunds-available)
- [Recording Services Outside YouTube](#recording-services-outside-youtube)
  * [Can I hire you for a recording outside YouTube?](#can-i-hire-you-for-a-recording-outside-youtube)
  * [What type of editing is included?](#what-type-of-editing-is-included)
- [You're a fraud, why do you ask for money, isn't YouTube Ads enough?](#youre-a-fraud-why-do-you-ask-for-money-isnt-youtube-ads-enough)

<!-- tocstop -->

## Membership Levels

I provide several different membership levels on YouTube and it tends to be a
bit confusing, so let's see what each one of them includes.

Pricing may vary depending on your country.

All paid memberships are managed by YouTube and can be joined from the same
page:

[View all YouTube memberships](https://www.youtube.com/channel/UCrSIvbFncPSlK6AdwE2QboA/join){:target="\_blank"}

<!-- markdownlint-disable -->
<!-- prettier-ignore-start -->
 
<!-- tip=green, info=blue, warning=yellow, danger=red -->
 
> Do not purchase a membership from an iPhone. YouTube already takes 30% of the membership, and Apple takes another 30% cut if you purchase through the iPhone app. Use a computer, Android device, or web browser instead.
{: .prompt-danger }
 
<!-- prettier-ignore-end -->
<!-- markdownlint-restore -->

For more details about livestreams, TTS, Discord, donations, and other channel
features, read this page:

[Livestream and channel information](https://linkarzu.com/posts/news/streaming-info/){:target="\_blank"}

The members-only Discord server can be joined here:

[https://discord.gg/D7CZMzw3rM](https://discord.gg/D7CZMzw3rM){:target="\_blank"}

Your Discord account must be connected to the same YouTube account that has the
active membership.

The **Executive Producer**, **CEO**, and **Board Member** levels are for the
**Linkarzu Root Access** podcast.

These are **group** podcast livestreams where members can join me and talk about
their workflow, computer setup, Linux, macOS, Neovim, Emacs, homelabbing,
networking, keyboards, business, YouTube channel, projects, videos they want us
to react to. Or anything else the group wants to discuss.

All **Linkarzu Root Access** episodes are published here:

[Listen to Linkarzu Root Access on YouTube](https://www.youtube.com/playlist?list=PLZWMav2s1MZR_E0s7OTicc7Th5QuWvUPx){:target="\_blank"}

You can use these episodes to:

- Regularly share progress on a project
- Show changes to your setup or workflow
- Ask for feedback from me and the community
- Discuss your business or YouTube channel
- Bring videos or topics for us to react to
- Spend time with other people in the community

Those episodes are not private one-on-one coaching calls. Other eligible members
may join the same episode, and the conversations are normally livestreamed
publicly so viewers can watch and participate through chat.

The episodes are scheduled based on my availability and the availability of the
members participating. If you cannot attend one of the scheduled episodes, let
me know and we can try to find another date when possible, but scheduling cannot
always be moved at the last minute.

The podcast is also distributed to Spotify, Apple Podcasts, YouTube Music, and
other podcast platforms.

### Member

For people who want to support the channel and get the basic community perks.

Price: **$4.19 per month**.

Includes:

- Loyalty badges next to your name in YouTube comments and livestream chats
- Access to the GOATED custom emojis
- Text To Speech during supported livestreams
- Access to the members-only Discord server

To use Text To Speech during a supported livestream, send your message like
this:

`!say I have a small pp`

Your message will be read out loud by a computer voice so I can hear it even
when chat is moving too fast.

TTS may be disabled during interviews or livestreams with guests so the
conversation is not interrupted every 14 seconds by someone asking which Linux
distribution is the best.

[Become a Member here](https://www.youtube.com/channel/UCrSIvbFncPSlK6AdwE2QboA/join){:target="\_blank"}

<!-- markdownlint-disable -->
<!-- prettier-ignore-start -->
 
<!-- tip=green, info=blue, warning=yellow, danger=red -->
 
> Do not purchase a membership from an iPhone. YouTube already takes 30% of the membership, and Apple takes another 30% cut if you purchase through the iPhone app. Use a computer, Android device, or web browser instead.
{: .prompt-danger }
 
<!-- prettier-ignore-end -->
<!-- markdownlint-restore -->

### Executive Producer

For people who want to join one **Linkarzu Root Access** group podcast episode
every month.

Price: **$24.99 per month**.

Includes:

- All perks from the previous membership level
- 1 Linkarzu Root Access podcast livestream per month

[Become an Executive Producer here](https://www.youtube.com/channel/UCrSIvbFncPSlK6AdwE2QboA/join){:target="\_blank"}

<!-- markdownlint-disable -->
<!-- prettier-ignore-start -->
 
<!-- tip=green, info=blue, warning=yellow, danger=red -->
 
> Do not purchase a membership from an iPhone. YouTube already takes 30% of the membership, and Apple takes another 30% cut if you purchase through the iPhone app. Use a computer, Android device, or web browser instead.
{: .prompt-danger }
 
<!-- prettier-ignore-end -->
<!-- markdownlint-restore -->

### CEO

For people who want to participate in **Linkarzu Root Access** more often.

Price: **$49.99 per month**.

Includes:

- All perks from the previous membership levels
- 2 Linkarzu Root Access podcast livestreams per month

[Become a CEO here](https://www.youtube.com/channel/UCrSIvbFncPSlK6AdwE2QboA/join){:target="\_blank"}

<!-- markdownlint-disable -->
<!-- prettier-ignore-start -->
 
<!-- tip=green, info=blue, warning=yellow, danger=red -->
 
> Do not purchase a membership from an iPhone. YouTube already takes 30% of the membership, and Apple takes another 30% cut if you purchase through the iPhone app. Use a computer, Android device, or web browser instead.
{: .prompt-danger }
 
<!-- prettier-ignore-end -->
<!-- markdownlint-restore -->

### Board Member

For people who want to join **Linkarzu Root Access** every week.

Price: **$99.99 per month**.

Includes:

- All perks from the previous membership levels
- 4 Linkarzu Root Access podcast livestreams per month

[Become a Board Member here](https://www.youtube.com/channel/UCrSIvbFncPSlK6AdwE2QboA/join){:target="\_blank"}

<!-- markdownlint-disable -->
<!-- prettier-ignore-start -->
 
<!-- tip=green, info=blue, warning=yellow, danger=red -->
 
> Do not purchase a membership from an iPhone. YouTube already takes 30% of the membership, and Apple takes another 30% cut if you purchase through the iPhone app. Use a computer, Android device, or web browser instead.
{: .prompt-danger }
 
<!-- prettier-ignore-end -->
<!-- markdownlint-restore -->

### Podcast Director

For people who want to run their own podcast without dealing with all the
annoying technical work behind it.

You bring the topic, you bring the guests, you run the conversation.

I take care of the livestream, layouts, recording, audio cleanup, editing,
publishing, podcast distribution, clips, Shorts, and all the other crap that
happens behind the scenes.

Price: **$499.99 per month**.

Includes:

- All perks from the previous membership levels
- 1 fully produced podcast episode every month
- Recording, livestream production, editing, publishing, podcast distribution,
  clips, and Shorts
- A dedicated section below with the full scope, rules, and scheduling details

[Become a Podcast Director here](https://www.youtube.com/channel/UCrSIvbFncPSlK6AdwE2QboA/join){:target="\_blank"}

<!-- markdownlint-disable -->
<!-- prettier-ignore-start -->
 
<!-- tip=green, info=blue, warning=yellow, danger=red -->
 
> Do not purchase a membership from an iPhone. YouTube already takes 30% of the membership, and Apple takes another 30% cut if you purchase through the iPhone app. Use a computer, Android device, or web browser instead.
{: .prompt-danger }
 
<!-- prettier-ignore-end -->
<!-- markdownlint-restore -->

## Podcast Director information

### Important information

#### What is included in the Podcast Director membership?

Every paid month includes one fully produced podcast episode:

- 1 podcast livestream
- Up to 2 hours of recording
- Up to 4 guests, plus you
- The option to invite me as one of your guests
- A dedicated podcast playlist on my YouTube channel
- A separate podcast RSS feed for your show
- Distribution to Spotify, Apple Podcasts, YouTube Music, and other podcast
  platforms
- 1 edited full podcast episode
- 1 edited video clip of approximately 10 minutes from the episode
- 5 edited vertical Shorts from the episode
- Access to all 4 monthly Linkarzu Root Access group livestreams
- All perks from the previous YouTube membership levels

The 2 hours refer to **2 hours of my recording and livestream production time**,
not a guaranteed final episode of exactly 2 hours. A standard episode usually
turns into at least 8 hours of work for me after preparation, editing,
exporting, publishing, and distribution.

#### Why does the Podcast Director membership cost that?

The $499.99 price is not just for 2 hours of recording. It covers the production
pipeline: livestream setup, recording up to 5 people, audio cleanup, editing,
uploading, podcast distribution, clips, Shorts, and access to my existing
audience.

My normal hourly rate is $50, and a Director episode usually requires at least 8
hours of total work. The membership also lets you launch a podcast using my
channel, workflow, and distribution setup instead of building everything from
zero.

If that's the value you're looking for, this membership may be a good fit.

If all you need is recording and editing for content that will live on your own
platforms, my recording services through Ko-Fi will probably be the better
option.

#### What are the benefits of becoming a Podcast Director?

Your episode is livestreamed and published on my YouTube channel, so you can use
my existing audience instead of starting from zero.

This gives you an opportunity to:

- Introduce yourself to my audience
- Promote your business, products, or services
- Share your YouTube channel or social media accounts
- Talk about your projects and ideas
- Build your own podcast without managing the technical production

I may also ask previous guests or community members if they want to join your
episode, but there are **no guarantees** they will accept or reply. I only make
those introductions if I am also joining the episode, so the conversation feels
natural.

### Running your podcast

#### Who runs the podcast episode?

You do.

You host the conversation, introduce the guests, choose the subjects, and decide
where the discussion goes. I handle the livestream and production in the
background unless you invite me to participate as a guest.

#### How many guests can I invite?

You can invite up to **4 guests**, plus yourself, for a maximum of 5 people. If
someone cancels, I adjust the layout and we continue with fewer people.

#### Can the podcast be about anything?

Pretty much. You can talk about your business, YouTube channel, open-source
project, Linux, Emacs, Neovim, networking, homelabbing, keyboards, productivity,
or your personal workflow. You can also promote your business, products,
services, projects, social accounts, Ko-Fi page, or YouTube channel.

However, the following content is not allowed:

- Illegal activity
- Pornography
- Drug promotion
- Gambling promotion
- Hate speech
- Harassment
- Scams
- Dangerous misinformation
- Copyrighted broadcasts
- Anything that could put my YouTube channel at risk

I retain final editorial and publishing control so I can comply with YouTube
rules, copyright requirements, and protect my channel.

#### Can we record with or without a live audience?

Yes. We can record in whichever format fits the episode:

- A public livestream
- A members-only livestream
- An unlisted livestream
- A private recording with no livestream

We'll decide the format before the recording.

#### What equipment do I and my guests need?

A dedicated microphone connected to a computer is strongly recommended, but a
phone can work if that is the only option.

**Headphones are mandatory** to avoid echo. Guest cameras are received at a
maximum resolution of 720p, though higher-quality cameras still help because the
image usually looks better after being downscaled.

### Publishing and editing

#### Where will my podcast be published?

Your episode is published on my YouTube channel inside its own dedicated podcast
playlist. You also receive a separate podcast RSS feed so the show can be
distributed to platforms such as:

- Spotify
- Apple Podcasts
- YouTube Music
- Amazon Music
- Pocket Casts
- Other podcast platforms

The goal is to distribute it everywhere possible. If a platform is missing, let
me know and I'll see if I can add it.

#### What editing is included?

The full recording is edited before publication. The standard edit includes:

- Removing unnecessary silences
- Removing technical interruptions
- Fixing and leveling guest audio
- Reducing distracting background noise when possible
- Cleaning up the beginning and ending
- Preparing the final video and audio versions
- Publishing the episode
- Distributing the episode to podcast platforms

Tell me before publication if a specific section should be removed. After the
episode is published, it will not be fully re-edited or taken down just because
someone changed their mind. If there is a valid reason, I may be able to remove
a specific section with the YouTube editor.

#### What additional videos do I receive?

Every episode includes:

- 1 edited video clip of approximately 10 minutes
- 5 edited vertical Shorts

You can receive these files and publish them on your own YouTube channel,
TikTok, Instagram, X, or wherever else you want.

You can also ask me to publish them on my channel instead.

The full podcast episode itself remains published on my channel unless we
arrange a separate production package outside the YouTube membership.

#### How long does publication take?

The edited episode should be ready **within 7 days**. New podcast feeds and
third-party platform listings may take longer because their approval and
processing times are outside my control.

#### Can I publish the full podcast episode on my own YouTube channel?

Not through the normal Podcast Director membership.

Under the Podcast Director membership, the full edited episode is published on
my YouTube channel and remains part of my channel's podcast catalog.

All revenue generated directly through my YouTube channel belongs to me,
including AdSense revenue, memberships, and Super Chats. You may still promote
your own Ko-Fi page, business, products, services, and social accounts during
the episode.

You can display your own Ko-Fi banner during the livestream so viewers can send
donations directly to you.

If you want the complete edited episode for publication on your own channel, we
can arrange the same production service separately through my Ko-Fi page:

[https://ko-fi.com/linkarzu](https://ko-fi.com/linkarzu){:target="\_blank"}

In that case, you receive the complete edited episode, the approximately
10-minute clip, and the 5 Shorts for publication on your own platforms.

### Scheduling

#### How far in advance should I schedule the episode?

Schedule the episode at least **1 week in advance** so we have time to:

- Choose a date that works
- Coordinate with all guests
- Prepare the livestream
- Collect guest names and information
- Prepare titles, graphics, and layouts
- Fix technical problems before going live

Everyone should join approximately **20 minutes before the livestream** so I can
check microphones, level audio, test cameras, and fix technical problems.

#### What happens if I do not use my episode that month?

Unused podcast episodes can roll over, but you can only accumulate a maximum of
**2 unused episodes**.

Once you have 2 unused episodes stored, you should schedule one before
accumulating another.

#### What happens if I cancel the membership?

Episodes that were already recorded and published remain online after the
membership is cancelled. They remain on my channel. Cancelling prevents you from
earning additional podcast episodes.

Any unused episodes earned before cancellation must be scheduled and recorded
within 60 days after the membership ends. You can have a maximum of 2 unused
episodes.

#### Can I purchase an additional recording hour?

Yes.

The normal membership includes up to 2 hours of recording, with an absolute
maximum of approximately **2 hours and 15 minutes**.

An additional hour can be purchased for **$200** through my Ko-Fi page:

[https://ko-fi.com/linkarzu](https://ko-fi.com/linkarzu){:target="\_blank"}

The additional hour extends the absolute recording limit to approximately **3
hours and 15 minutes**.

The rate is **$200 per additional recorded hour**, not per hour of editing. The
extra hour should preferably be arranged and paid before the livestream.
Same-day extensions are only possible if I have availability.

### Final notes

#### Are views, subscribers, clients, or sales guaranteed?

No. This membership gives you access to my production workflow, my channel, and
my existing audience, but it does not guarantee:

- A specific number of views
- New subscribers
- Donations
- Clients
- Product sales
- Business leads
- Viral success

I will prepare and publish the content, but audience reaction and platform
performance cannot be guaranteed.

#### Are refunds available?

Membership payments and refunds are managed by YouTube. You will need to contact
YouTube if you want to request a refund, because I do not manually control
YouTube membership refunds.

## Recording Services Outside YouTube

### Can I hire you for a recording outside YouTube?

Yes.

The Podcast Director membership is for shows published on my YouTube channel. If
you need the same recording and production setup for content that belongs to you
or your business, we can arrange it separately through Ko-Fi:

[https://ko-fi.com/linkarzu](https://ko-fi.com/linkarzu){:target="\_blank"}

Rate: **$200 per recorded hour**.

The price is based on recording time, not the number of hours I spend editing
afterward. One recorded hour may still require several additional hours of
preparation, audio cleanup, editing, exporting, and delivery.

For example:

- Business interviews
- Tutorials, demonstrations, walkthroughs, or training videos
- Remote presentations with multiple guests
- Screen-sharing sessions with dynamic layout switching
- Professionally produced conversations that do not publish on my channel

For work arranged outside YouTube, we can define what you need before recording,
including:

- Whether the session will be live or prerecorded
- Who will participate
- Whether screen sharing is required
- What platforms or layouts are needed
- What editing and deliverables are included
- Where the finished content will be published
- Whether you need clips or Shorts

Unlike the Podcast Director membership, the finished content can be delivered to
you for publication on your own platforms.

### What type of editing is included?

Whether you're a Podcast Director member or hiring me through Ko-Fi, the
standard edit is similar to what you already see on my
[Podcast YouTube channel](https://www.youtube.com/@linkarzu-podcast){:target="\_blank"}

It includes:

- Audio cleanup and leveling
- Removing unnecessary silences
- Basic cuts
- Switching between cameras and screen shares
- Exporting the final video
- Publishing and podcast distribution when applicable

The service does **not** include:

- Motion graphics
- Custom animations
- Complex visual effects
- Heavy restructuring of the recording
- Multiple revision rounds
- Custom branding packages
- Thumbnail design (unless previously agreed)
- Anything else that significantly increases editing time

The goal is a clean, professional recording, not a full Hollywood production. If
your project needs something outside the standard edit, we'll define it before
recording so we both know exactly what's included.

## You're a fraud, why do you ask for money, isn't YouTube Ads enough?

- I explain all of this in the "about me page" link below:
  - [youre-a-fraud-why-do-you-ask-for-money-isnt-youtube-ads-enough](https://linkarzu.com/about/#youre-a-fraud-why-do-you-ask-for-money-isnt-youtube-ads-enough){:target="\_blank"}
  - Above you'll also find links to my discord, social media, etc

